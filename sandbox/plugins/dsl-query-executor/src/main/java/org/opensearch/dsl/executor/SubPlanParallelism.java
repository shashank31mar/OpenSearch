 ...
 * K_eff    = clamp(min(n - 1, kSetting, [K_gate], [K_search]), 1, n - 1)
 * </pre>
 ...
 * indistinguishable from a real one and would clamp where the contract says drop. {@code n - 1} and
...
public final class SubPlanParallelism {
    ...
    public static Decision decide(Inputs in) {
        ...
        // n <= 1 never reaches a fan-out decision, and clamp(..., 1, n - 1) would be malformed there
        // (upper < lower). Returning 1 also keeps the caller from ever constructing a
        // PendingExecutions(0), whose constructor asserts permits > 0.
        if (in.n() <= 1) {
            return new Decision(1, a, f, kGate, kSearch);
        }

        int kEff = Math.min(in.n() - 1, Math.max(1, Math.min(MAX_K_SETTING, in.kSetting())));
        if (kGate.isPresent()) {
        ...
        // callers rely on (a gate of width 0 or a width above the remaining plan count).
        kEff = Math.max(1, Math.min(kEff, in.n() - 1));
