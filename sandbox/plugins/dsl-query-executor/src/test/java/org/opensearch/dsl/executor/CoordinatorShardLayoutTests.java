 ...
 *
 * <p>Cluster states come from {@link ShardLayouts}, which places each shard explicitly — see its javadoc
 * for why the framework's random-placement helper cannot be used for these assertions. The same fixture
 * wires the read into the executor in
 * {@code DslQueryPlanExecutorTests#testShardLayoutTermIsReadFromLiveRoutingNotAConstant}, so the
 * function and its call site are pinned against one shard layout.
 */
