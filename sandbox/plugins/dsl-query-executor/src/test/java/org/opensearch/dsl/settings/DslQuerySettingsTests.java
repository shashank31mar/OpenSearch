...
public class DslQuerySettingsTests extends OpenSearchTestCase {
    ...
    /**
     * Guards against a second setting being added without {@code getSettings()} being updated, and
     * against {@code dsl.query.enabled} being re-introduced here: the interception gate belongs to
     ...
     */
    public void testAllContainsOnlyTheWidthSetting() {
        List<Setting<?>> all = DslQuerySettings.all();
        assertEquals("all() must contain exactly the fan-out width setting, got " + keysOf(all), 1, all.size());
