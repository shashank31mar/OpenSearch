 ...
 *
 * <p>Built one shard at a time on purpose. The framework's
 * {@code ClusterStateCreationUtils.state(index, nodes, primaries)} assigns each primary with
 * {@code randomFrom(nodes)}, so the skew these tests turn on would be random and their assertions
 * non-deterministic.
 */
