...
public class DslGateInputsTests extends OpenSearchTestCase {
    ...
    /** Same key/type/bounds/properties as {@code DatafusionSettings.CONCURRENCY_DATANODE_MULTIPLIER}. */
    ...
    /** Same key/type/bounds/properties as {@code AnalyticsQuerySettings.MAX_CONCURRENT_SHARD_REQUESTS_PER_NODE}. */
     ...
     * defends: the declaring plugin loosening its own minimum, invisible here because the read is by
     * string. The positive half is asserted too, so the guard cannot be widened into a "below 1" clamp.
     */
